...
Key requirements:
1. StatefulSet with spec.serviceName referencing the headless service
2. volumeClaimTemplates for /var/lib/one (persistence.size from values)
3. InitContainer to wait for MariaDB (when enabled)
4. Environment variables wired from secrets and helpers
...
**CRITICAL: spec.serviceName must be set:**
```yaml
spec:
...
**CRITICAL: Environment variables must be properly wired:**

```yaml
env:
  - name: ONEADMIN_PASSWORD
    valueFrom:
      secretKeyRef:
        name: {{ include "opennebula.fullname" . }}-credentials
        key: oneadmin-password
  - name: DB_BACKEND
    value: "mysql"
  - name: DB_HOST
    value: {{ include "opennebula.mariadb.host" . | quote }}
  - name: DB_PORT
    value: {{ include "opennebula.mariadb.port" . | quote }}
  - name: DB_USER
    value: {{ include "opennebula.mariadb.username" . | quote }}
  - name: DB_NAME
    value: {{ include "opennebula.mariadb.database" . | quote }}
  - name: DB_PASSWORD
    valueFrom:
      secretKeyRef:
        name: {{ include "opennebula.mariadb.secretName" . }}
...
Mount config from configmap to /etc/one/ (optional override).
  ...
  <verify>
```bash
helm template test charts/opennebula --set mariadb.enabled=false --set externalDatabase.host=db.local 2>&1 | grep -q "kind: StatefulSet" && echo "PASS" || echo "FAIL"
# Also verify serviceName is set:
helm template test charts/opennebula --set mariadb.enabled=false --set externalDatabase.host=db.local 2>&1 | grep -q "serviceName:" && echo "serviceName PASS" || echo "serviceName FAIL"
# And verify DB_PASSWORD secretKeyRef:
helm template test charts/opennebula --set mariadb.enabled=false --set externalDatabase.host=db.local 2>&1 | grep -A3 "name: DB_PASSWORD" | grep -q "secretKeyRef" && echo "DB_PASSWORD PASS" || echo "DB_PASSWORD FAIL"
```
  </verify>
  <done>StatefulSet template creates pod with spec.serviceName, volumeClaimTemplates, init container, liveness/readiness probes, and environment from secrets including DB_HOST and DB_PASSWORD via secretKeyRef.</done>
</task>
...
StatefulSet serviceName must reference the headless service name.
  ...
  <verify>
```bash
helm template test charts/opennebula --set mariadb.enabled=false --set externalDatabase.host=db.local 2>&1 | grep -c "kind: Service"
```
...
**SSH Key Persistence Solution (HELM-03 compliant):**
The container generates SSH keys on first start, but these must persist across pod restarts. Options:
1. User provides ssh.privateKey/publicKey in values (preferred for production)
2. Mount /var/lib/one/.ssh directory from the PersistentVolume (keys generated by entrypoint persist with data)

Since the StatefulSet already has a PV for /var/lib/one, SSH keys in /var/lib/one/.ssh/ are automatically persisted. The secret stores user-provided keys OR acts as initial seed.
...
For SSH keys secret (separate secret for optional user-provided keys):
```yaml
{{- if or .Values.ssh.privateKey .Values.ssh.publicKey }}
---
apiVersion: v1
kind: Secret
metadata:
...
**Key insight:** SSH keys persist via the PV mount at /var/lib/one (which includes .ssh/). If users want to provide their own keys, they use the ssh.privateKey/publicKey values which mount as a secret. Otherwise, entrypoint generates keys that persist in the PV.
...
For now, ConfigMap can be minimal - entrypoint.sh handles DB config dynamically.
  ...
  <verify>
```bash
helm template test charts/opennebula --set mariadb.enabled=false --set externalDatabase.host=db.local 2>&1 | grep -E "kind: (Secret|ConfigMap)" | wc -l
```
Should output at least 2 (one Secret, one ConfigMap).
  </verify>
  <done>Secret stores admin password with lookup persistence. SSH keys persist via PV mount at /var/lib/one/.ssh, with optional user-provided keys via separate secret. ConfigMap provides config file management.</done>
</task>
...
```bash
# Lint the chart
helm lint charts/opennebula --set mariadb.enabled=false --set externalDatabase.host=db.local

# Template renders without errors
helm template test charts/opennebula --set mariadb.enabled=false --set externalDatabase.host=db.local

# Verify all resources present
helm template test charts/opennebula --set mariadb.enabled=false --set externalDatabase.host=db.local | grep "kind:" | sort | uniq -c

# Verify DB_HOST and DB_PASSWORD are properly wired
helm template test charts/opennebula --set mariadb.enabled=false --set externalDatabase.host=db.local | grep -A2 "name: DB_HOST"
helm template test charts/opennebula --set mariadb.enabled=false --set externalDatabase.host=db.local | grep -A4 "name: DB_PASSWORD"
```
...
<success_criteria>
- StatefulSet template with spec.serviceName set to headless service name
- StatefulSet template with volumeClaimTemplates for HELM-01
- Environment variables properly wired: DB_HOST via helper, DB_PASSWORD via secretKeyRef
...
- Secret uses lookup function to persist credentials across upgrades for HELM-03
- SSH keys persist via PV at /var/lib/one/.ssh, with optional user-provided keys
